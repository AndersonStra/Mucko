...
class OkvqaTrainDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        with open(config["dataset"]["train"]["train_qa_raw"], 'r') as f:
            qa_raw = json.load(f)
        ...
        sem_data = np.load(config["dataset"]["train"]["train_semantic_graph"])
        ...
        fact_data = np.load(config["dataset"]["train"]["train_facts_graph"])
